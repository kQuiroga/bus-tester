...
- [x] 4.6 C5 FIDELITY (prototype `docs/redesign-prototype/Main.dc.html` LIVE card, Strict TDD — 13 RED in `messages.component.spec.ts` then GREEN 235/235; `npm run build` OK): `.qpill` chip is now a dark tint `color-mix(in oklab, var(--queue-hue) 14%, var(--color-ground))` with text colour `var(--queue-hue)` (was 18%/transparent + card-foreground); `.dot` `rounded-full` → `rounded-[3px]` (rounded square). Subscription chips render `● name · count ✕` as a single `.qpill` with a plain `✕` button (dropped `hlmBadge` + separate count badge + `lucideX`); empty state `Ninguna cola suscrita.`. Subscribe row = `.in` input (`h-[34px] rounded-[8px] bg-muted`, placeholder/aria "Nombre de la cola") + `.btn-sm` "Suscribirse". Search row = `.in` input + `.ghost` "Pausar"/"Reanudar" (icons dropped: `lucideSearch`/`lucidePause`/`lucidePlay` removed). `.msg` cards → `bg-panel-2 border-border rounded-[9px] px-[11px] py-[9px]`; header = `.qpill` + routing key `font-mono text-xs font-medium` (dropped the `(exchange)` suffix); "Responder" is a `.ghost` button with `text-accent`; meta line `exchange: {value}` (`metaExchange()` → `(intercambio predeterminado)` when empty, C4 wording); payload `font-mono text-[11px] text-muted-foreground`. Feed empty states via `emptyFeedMessage()`: "Suscribite a una cola para ver mensajes en vivo." / "Esperando mensajes…" / "Ningún mensaje coincide con la búsqueda." (`.empty` dashed box). Card padding `p-4` → `p-[18px]`. `queue-color.ts` / `queue-color.spec.ts` (FNV-1a + pinned values) untouched. Slice-5 reply-drawer / `data-replying` regions untouched.
...
- [x] 5.11 C6 FIDELITY (prototype `docs/redesign-prototype/Main.dc.html` reply drawer ~157-178, Strict TDD — `reply-drawer.component.spec.ts` 12→17, RED = compile failure on `origin.queue` + 5 pending assertions, then GREEN; full suite 244/244; `npm run build` OK): `hlm-sheet-content` → `bg-card p-[18px] sm:max-w-[420px]` with `[showCloseButton]="false"`; header is `[● dot Responder]` — dot `size-1.5 rounded-[3px]` filled with `var(--queue-hue)` — plus a plain `✕` close button (`data-testid="reply-close"`). "Mensaje original" box → `rounded-[8px] border border-border border-l-2 bg-muted px-3 py-2.5` with `[style.border-left-color]="var(--queue-hue)"`, containing `.field-label` + mono routing key + mono meta (`originMeta()` → `exchange: {value}` / `(intercambio predeterminado)`) + mono payload. Correlation ID is now an always-rendered readonly `.in` (`h-[34px] rounded-[8px] bg-muted font-mono text-muted-foreground`, `data-testid="reply-correlation-id"`) — dropped the `@if`, the separate routing-key input, the exchange chip and the Cancelar button. Payload = `.field-label` + textarea `min-h-[120px] rounded-[8px] bg-muted`. Send = full-width `hlmBtn` `h-[38px] rounded-[9px]` "Enviar respuesta"; muted `text-[11px]` caption "El panel de Enviar queda intacto con tu borrador.". Hue plumbing: `ReplyTarget.origin` gains `queue`; `MessagesComponent.respond()` fills it via `queueNameOf(message)`; drawer `queueColor()` = `queueColorIndex(origin.queue)` → `[attr.data-queue-color]` on the sheet content (slice-4 `--queue-hue` mechanism). Drawer behavior (opens on `respond()`, `clear()` on close, own `POST /api/messages`, empty-exchange AMQP-default validation) unchanged. `styles.css` untouched. Commit `efc7a7d`.
